  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 mb-1">
        ...
        <span className="font-semibold text-xs text-foreground">{data.id}</span>
        {!data.enabled && <span className="rounded bg-chart-5/10 px-1 text-[8px] text-chart-5">off</span>}
      </div>
      ...
      {expanded && (
        <div className="mt-2 pt-2 border-t border-border/30 text-[9px] space-y-0.5">
          <div><span className="text-muted-foreground/50">Cron: </span><span className="font-mono text-foreground">{data.cron}</span></div>
          ...
          {data.workflow_ref && <div><span className="text-muted-foreground/50">Triggers: </span><span className="text-foreground">{data.workflow_ref}</span></div>}
        </div>
      )}
